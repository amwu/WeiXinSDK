            ...
            switch (message.Type)
            {
                case WeixinMessageType.Text://文字消息
                    ...
                    if (userMessage.Equals("订阅"))
                    {
                        ...
                        list.Add(new WeixinNews { title = "新闻3", description = "欢迎订阅，点击此消息查看在线demo", picurl = string.Format("{0}/Images/ad.jpg", domain), url = domain });
                        list.Add(new WeixinNews { title = "新闻4", description = "欢迎订阅，点击此消息查看在线demo", picurl = string.Format("{0}/Images/ad.jpg", domain), url = domain });
                        result = ReplayPassiveMessageAPI.RepayNews(openId, myUserName, list);


                    }
                    else if (userMessage.Equals("菜单"))
                    {
                ...
                case WeixinMessageType.Shortvideo://短视频
                    {
                            ...
                            #region 自定义菜单事件
                            {
                                switch (eventKey)
                                {
                                    ...
                                    case "binduser"://绑定用户
                                        ...
                                        sb.Append("<a href=\"");//HttpUtility.UrlEncode

                                        //sb.Append(OAuth2API.GetAuthorizeUrl(WeixinConfig.AppID, "http://bjflxd.eicp.net/oauth2", "123", OAuth2API.OAuthScope.snsapi_userinfo, "code"));
                                        ...
                                        sb.Append(weixinOAuth2Url);

                                        sb.Append("\">点击这里体验</a>");

                                        // urlencode

                                        result = ReplayPassiveMessageAPI.RepayText(openId, myUserName, sb.ToString());


                                        break;
                                    case "V1001_GOOD"://赞一下我们



                                        result = ReplayPassiveMessageAPI.RepayText(openId, myUserName, "已赞");
                                        break;
